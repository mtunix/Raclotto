...
from back.src.api.serializer import (
...
from back.src.entity.ingredient import (
...
class PanApi(BaseApi):
    ...
    def generate(self):
        ...
        num_sauce = (
            min(num_sauce, len(sauces)) if len(sauces) >= num_sauce else len(sauces)
        )
        ...
        existing_join = (
            db.session.query(user_sessions)
            .filter(
            ...
            try:
                db.session.execute(
                    user_sessions.insert().values(
                        user_id=user.id, session_id=session.id, joined_at=datetime.now()
                    )
                )
                db.session.flush()
            except Exception:
        ...
        newly_unlocked_achievements = achievement_service.evaluate_achievements_for_pan(
            pan, user
        )
        ...
        if newly_unlocked_achievements:
            achievements_data = serialize_collection(
            ...
            if achievements_data and "data" in achievements_data:
                ...
                pan_response["meta"]["newly_unlocked_achievements"] = achievements_data[
                    "data"
                ]
            ...
            if available_fill_count > 0 and available_sauce_count > 0:
                ...
                if remaining < 0:
                    ...
                    if total_selected < total_columns:
                        ...
                        while remaining > 0:
                            if (
                                num_fill < available_fill_count
                                and num_sauce < available_sauce_count
                            ):
                                ...
                                num_fill += min(
                                    remaining, available_fill_count - num_fill
                                )
                                remaining -= min(
                                    remaining, available_fill_count - num_fill
                                )
                                ...
                                num_sauce += min(
                                    remaining, available_sauce_count - num_sauce
                                )
                                remaining -= min(
                                    remaining, available_sauce_count - num_sauce
                                )
    ...
    def list_pans(self):
        ...
        if session_key:
            ...
            try:
                ...
                session = (
                    db.session.query(RaclottoSession).filter_by(key=session_key).one()
                )
                total_count = (
                    db.session.query(func.count(Pan.id))
            ...
            # Get total count for pagination metadata
            total_count = (
                db.session.query(func.count(Pan.id)).filter_by(user_id=user_id).scalar()
                or 0
            )
        else:
        ...
        for pan in pans:
            ...
            if pan_dict and pan_dict.get("attributes"):
                ...
                if user:
                    ...
                    pan_dict["attributes"]["user_color"] = (
                        user.color if user.color else "#1890ff"
                    )
                    pan_dict["attributes"]["user_id"] = user.id
                    pan_dict["attributes"]["user_profile_picture"] = (
                        user.get_profile_picture()
                    )
                    pan_dict["attributes"]["user_border_style"] = (
                        user.border_style if user.border_style else "solid"
                    )
                    pan_dict["attributes"]["user_border_texture"] = (
                        user.border_texture if user.border_texture else None
                    )
                    pan_dict["attributes"]["user_glow_effect"] = (
                        bool(user.glow_effect)
                        if user.glow_effect is not None
                        else False
                    )
                else:
        ...
        response = {"jsonapi": {"version": JSONAPI_VERSION}, "data": enriched_pans}
    ...
    def get_pan(self, pan_id: int):
        ...
        if pan_dict and pan_dict.get("attributes"):
            ...
            if user:
                ...
                pan_dict["attributes"]["user_glow_effect"] = (
                    user.glow_effect if user.glow_effect else False
                )
            else:
                pan_dict["attributes"]["user"] = "Unknown"
                pan_dict["attributes"]["user_color"] = "#d9d9d9"
        ...
        return {"jsonapi": {"version": JSONAPI_VERSION}, "data": pan_dict}
    ...
    def create_pan(self):
        ...
        if "name" not in attributes or not attributes["name"]:
            ...
            r = RandomWord()
            attributes["name"] = (
                f"{r.word(include_parts_of_speech=['adjectives']).capitalize()} Raclotto Pan"
            )
