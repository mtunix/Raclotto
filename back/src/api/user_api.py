...
class UserApi(BaseApi):
    ...
    def get_user_stats(self, user_id: int):
        ...
        return {
            ...
            "data": {"type": "userStats", "id": str(user_id), "attributes": stats},
        }
    ...
    def _calculate_user_stats(self, user_id: int) -> Dict[str, Any]:
        ...
        if user_pans:
            ...
            avg_rating_result = (
                ...
                .filter(Rating.pan_id.in_(pan_ids))
                .scalar()
            )
            average_rating_received = (
                float(avg_rating_result) if avg_rating_result else 0.0
            )
        else:
        ...
        if user_pans:
            ...
            for pan in user_pans:
                for ingredient in pan.ingredients:
                    ...
                    ingredient_counts[ingredient_id] = (
                        ingredient_counts.get(ingredient_id, 0) + 1
                    )

            if ingredient_counts:
                most_used_id = max(ingredient_counts.items(), key=lambda x: x[1])[0]
                favorite_ingredient_obj = (
                    db.session.query(Ingredient).filter_by(id=most_used_id).first()
                )
                if favorite_ingredient_obj:
                    favorite_ingredient = {
                        "id": favorite_ingredient_obj.id,
                        "name": favorite_ingredient_obj.name,
                        "type": favorite_ingredient_obj.type.value
                        if hasattr(favorite_ingredient_obj.type, "value")
                        else str(favorite_ingredient_obj.type),
                    }
        ...
        if user_pans:
            ...
            ingredient_ratings = (
                ...
                .join(Pan, Pan.id == pan_ingredients.c.pan_id)
                .join(Rating, Rating.pan_id == Pan.id)
                .filter(Pan.id.in_(pan_ids))
                .group_by(pan_ingredients.c.ingredient_id)
            ...
            if ingredient_ratings:
                ...
                ingredient = (
                    db.session.query(Ingredient).filter_by(id=ingredient_id).first()
                )
                if ingredient:
                    best_rated_ingredient = {
                        ...
                        else str(ingredient.type),
                        "avg_rating": float(ingredient_ratings[1])
                        if ingredient_ratings[1]
                        else 0.0,
                    }
        ...
        total_achievement_points = (
            sum(achievement.value for achievement in user.achievements) if user else 0
        )

        # 8. Average ingredients per pan
        average_ingredients_per_pan = 0.0
        if user_pans:
            total_ingredients = sum(len(pan.ingredients) for pan in user_pans)
            average_ingredients_per_pan = total_ingredients / total_pans

        # 9. Most pans session (session where user created the most pans)
        most_pans_session = None
        if user_pans:
            session_pan_counts = {}
            for pan in user_pans:
                if pan.session_id:
                    session_pan_counts[pan.session_id] = (
                        session_pan_counts.get(pan.session_id, 0) + 1
                    )

            if session_pan_counts:
                max_session_id = max(session_pan_counts.items(), key=lambda x: x[1])[0]
                session = (
                    db.session.query(RaclottoSession)
                    .filter_by(id=max_session_id)
                    .first()
                )
                if session:
                    most_pans_session = {
                        ...
                        "pan_count": session_pan_counts[max_session_id],
                    }

        # 10. Sessions participated (number of unique sessions user has created pans in)
        sessions_participated = 0
        if user_pans:
            unique_sessions = set(pan.session_id for pan in user_pans if pan.session_id)
            sessions_participated = len(unique_sessions)

        # 11. Pan with most ingredients
        most_ingredients_pan = None
        if user_pans:
            pan_with_most = max(user_pans, key=lambda p: len(p.ingredients))
            if pan_with_most:
                most_ingredients_pan = {
                    "id": pan_with_most.id,
                    "name": pan_with_most.name,
                    "ingredient_count": len(pan_with_most.ingredients),
                    "ingredients": [
                        {
                            "id": ing.id,
                            "name": ing.name,
                            "type": ing.type.value
                            if hasattr(ing.type, "value")
                            else str(ing.type),
                        }
                        for ing in pan_with_most.ingredients
                    ],
                }
        ...
        return {
            ...
            "average_ingredients_per_pan": round(average_ingredients_per_pan, 2),
            "most_pans_session": most_pans_session,
            "sessions_participated": sessions_participated,
            "most_ingredients_pan": most_ingredients_pan,
        }
