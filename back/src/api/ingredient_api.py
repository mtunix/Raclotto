...
class IngredientApi(BaseApi):
    ...
    def list_ingredients(self):
        ...
        return {"jsonapi": {"version": "1.0"}, "data": data}
        ...
        if ingredient.type == IngredientType.SAUCE:
            ...
            if king_achievement:
                ...
                if not king_achievement.is_global:
                    ...
                    user_ids_with_achievement = (
                        self.user_repository.get_users_with_achievement(
                            king_achievement.id
                        )
                    )
                    user_ids_in_session = self.user_repository.get_users_in_session(
                        session.id
                    )

                    # Only re-evaluate users who have the achievement AND are in this session
                    users_to_re_evaluate = (
                        user_ids_with_achievement & user_ids_in_session
                    )
                else:
                ...
                for user_id in users_to_re_evaluate:
                    ...
                    if user:
                        ...
                        eval_session = (
                            session
                            if not king_achievement.is_global
                            else (user.sessions[0] if user.sessions else session)
                        )
    ...
    def get_available_counts(self):
        ...
        :returns Dict: Dictionary with 'fill_count', 'sauce_count', 'total_fill_count', and 'total_sauce_count'
        :status_code 200: Success
        :status_code 401: Not authenticated
        ...
        # Get total counts (all available ingredients regardless of dietary restrictions)
        total_fill_count = len(
            [i for i in ingredients if i.available and i.type == IngredientType.FILL]
        )
        total_sauce_count = len(
            [i for i in ingredients if i.available and i.type == IngredientType.SAUCE]
        )
        ...
        fill_count = len(
            [i for i in available_ingredients if i.type == IngredientType.FILL]
        )
        sauce_count = len(
            [i for i in available_ingredients if i.type == IngredientType.SAUCE]
        )
        ...
        return {
            ...
            "total_sauce_count": total_sauce_count,
        }
    ...
    def reimport_ingredients(self):
        ...
        current_sauce_ids = {
            ing.id for ing in current_ingredients if ing.type == IngredientType.SAUCE
        }
        deleted_sauce_ids = ids_to_delete & current_sauce_ids
        new_sauces_count = sum(
            1 for item in items_to_create if item.get("type") == IngredientType.SAUCE
        )
        ...
        created_ingredients = []
        if items_to_create:
            created_ingredients = self.repository.bulk_create(
                items_to_create, session.id
            )
