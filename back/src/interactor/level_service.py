...
class LevelService:
    ...
    def update_user_level(self, user: User) -> Optional[Level]:
        ...
        """
        # Cast experience_points to int in case it's a SQLAlchemy Column
        xp = cast(int, user.experience_points)
        new_level = self.calculate_level_from_xp(xp)

        if new_level:
            old_level_id = cast(Optional[int], user.level_id)
            user.level_id = new_level.id

            # Check if user leveled up
            if old_level_id is not None and old_level_id != new_level.id:
                db.session.flush()
                return new_level
    ...
    def award_xp_for_pan(self, user: User, pan: Pan) -> int:
        ...
        has_wildcard = any(bool(ing.wildcard) for ing in pan.ingredients)
        has_spicy = any(int(ing.spicy) >= 2 for ing in pan.ingredients)
        has_sweet = any(bool(ing.sweet) for ing in pan.ingredients)
        ...
        if bool(pan.rolled_preparation_type):
            multiplier *= 1.1
        if bool(pan.rolled_cheese):
            multiplier *= 1.2

        # Calculate final XP (rounded to integer)
        total_xp = int(base_total * multiplier)

        # Award XP (cast to int to handle SQLAlchemy Column type)
        current_xp = int(user.experience_points)  # type: ignore
        user.experience_points = current_xp + total_xp  # type: ignore
