...
class SurvivalOfTheFittestEvaluator(AchievementEvaluator):
    """
    Evaluator for "Survival of the Fittest" achievement.
    Description: "Esse die schlecht bewerteste Pfanne eines Raclottos (mindestens 2 Bewertungen)"
    (Eat the worst rated pan of a raclotto session, must have at least 2 ratings)
    
    This achievement is session-specific. It checks if the user has created
    the worst rated pan in the current session, and the pan must have been
    ...
    def _is_worst_rated_pan(self, pan: Pan, session_pans: List[Pan]) -> bool:
        """
        Check if this pan is the worst rated pan in the session.
        ...
        :return: True if this pan is the worst rated (lowest average rating)
        """
        ...
        for other_pan in session_pans:
            ...
            # If any other pan has a lower or equal rating, this pan is not the worst
            if other_rating < pan_rating:
                return False
        
        # This pan has the lowest (or tied for lowest) rating among pans with enough ratings
        return True
    ...
    def evaluate(
        ...
        """
        Check if the user has created the worst rated pan in the session.
        ...
        :return: EvaluationResult with unlocked=True if pan is worst rated with >=2 ratings
        """
        ...
        # Check if this pan is the worst rated in the session
        unlocked = self._is_worst_rated_pan(pan, context.session_pans)
    ...
    def calculate_progress(
        ...
        # Check if user has created the worst rated pan in the session
        ...
        for pan in user_pans_in_session:
            ...
            # Check if this pan is the worst rated
            if self._is_worst_rated_pan(pan, context.session_pans):
                return 1.0
