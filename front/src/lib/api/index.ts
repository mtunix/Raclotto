import { get, getRaw, post, patch, del, getHeaders } from "./api";
...
export class Api {
  ...
  static async getSessionByKey(sessionKey: string): Promise<RaclottoSession> {
    ...
    if (response.data && typeof response.data === "object") {
      ...
      return RaclottoSession.fromParsed({
        ...
        timestamp: (attrs as any).timestamp
          ? new Date((attrs as any).timestamp)
          : new Date(),
        active:
          (attrs as any).active !== undefined ? (attrs as any).active : true,
      });
    }
  ...
  static async add(
    session: string,
    data: Omit<Ingredient, "id" | "available">,
    resourceType: string = "ingredient",
  ): Promise<Ingredient | PrepType> {
    const endpoint =
      resourceType === "preparation_type"
  ...
  static async delete(
    ...
    item:
      | Ingredient
      | PrepType
      | { type?: IngredientType; name?: string; id: number },
    ...
    if (response.data && typeof response.data === "object") {
      ...
      if (responseData.data && responseData.data.attributes) {
        return {
          ...
          id: responseData.data.id || ingredientId,
        } as Ingredient;
      }
      return response.data as Ingredient;
    }
  ...
  static async generate(
    ...
    if (
      response &&
      response.meta &&
      (response.meta as any).newly_unlocked_achievements
    ) {
      const achievementData = (response.meta as any)
          ...
          value:
            item.attributes?.value !== undefined
              ...
              : item.value !== undefined
                ...
                : 0,
          hidden:
            item.attributes?.hidden !== undefined
  ...
  static async generateBandit(
    session: string,
    totalColumns: number,
    rollPrepType: boolean,
    rollCheese?: boolean,
  ): Promise<GenerationResponse> {
  ...
  static async getPansPaginated(
    session: string,
    limit: number = 12,
    offset: number = 0,
  ): Promise<{ data: Pan[]; hasMore: boolean; total: number }> {
    ...
    const pans = dataArray.map((item: any) => {
      ...
      return {
        ...
        user_id:
          attributes.user_id !== undefined
            ? typeof attributes.user_id === "string"
              ? parseInt(attributes.user_id, 10)
              : Number(attributes.user_id)
            : item.user_id,
        user_profile_picture:
          attributes.user_profile_picture || item.user_profile_picture,
        user_border_style:
          attributes.user_border_style || item.user_border_style,
        user_border_texture:
          attributes.user_border_texture !== undefined
            ? attributes.user_border_texture
            : item.user_border_texture,
        user_glow_effect:
          attributes.user_glow_effect !== undefined
            ? attributes.user_glow_effect
            : item.user_glow_effect !== undefined
              ? item.user_glow_effect
              : false,
    ...
    const hasMore =
      meta.has_more !== undefined
        ? meta.has_more
        : offset + pans.length < (meta.total || 0);
  ...
  static async getStats(session?: string): Promise<StatsResponse> {
    ...
    // getRaw returns the raw response, which has a data property containing the stats
    if (response && response.data && typeof response.data === "object") {
      const statsData = response.data;

      // Extract the necessary fields from the response
      return {
        pans: Array.isArray(statsData.pans) ? statsData.pans : [],
        ingredients_top_rated: Array.isArray(statsData.ingredients_top_rated)
          ? statsData.ingredients_top_rated
          : [],
        ingredients_most_used: Array.isArray(statsData.ingredients_most_used)
          ? statsData.ingredients_most_used
          : [],
        leaderboard: Array.isArray(statsData.leaderboard)
  ...
  static async getAvailableIngredientCounts(session: string): Promise<{
    fill_count: number;
    sauce_count: number;
    total_fill_count: number;
    total_sauce_count: number;
  }> {
    const endpoint = `${API_BASE}/ingredients/available-counts?session_key=${session}`;
    const response = await getRaw(endpoint);
    if (response && typeof response === "object") {
      // The response is wrapped in a data object: { data: { fill_count, sauce_count, total_fill_count, total_sauce_count }, ... }
      const wrappedData = response as {
        data?: {
          fill_count?: number;
          sauce_count?: number;
          total_fill_count?: number;
      ...
      if (data && typeof data === "object") {
        return {
          ...
          total_fill_count: data.total_fill_count || 0,
          total_sauce_count: data.total_sauce_count || 0,
        };
      }
    }
  ...
  static async refreshToken(): Promise<string> {
    ...
    if (
      ...
      "attributes" in response.data.data
    ) {
  ...
  /**
   * Fetch user statistics including:
   * - total_pans: Number of pans created by the user
   * - total_ratings_given: Number of ratings given by the user
   * - average_rating_received: Average rating received on user's pans
   * - total_achievement_points: Total achievement points earned
   * - favorite_ingredient: Most frequently used ingredient
   * - best_rated_ingredient: Highest rated ingredient created
   * - pans_eaten: Number of pans the user has eaten/tasted
   * - average_ingredients_per_pan: Average number of ingredients in user's pans
   * - most_pans_session: Object containing session details with most pans:
   *   - session_id: ID of the session
   *   - session_name: Name of the session
   *   - pan_count: Number of pans in that session
   * - sessions_participated: Number of sessions the user has participated in
   */
  ...
  static async listInvites(): Promise<any[]> {
    ...
    return dataArray.map((item: any) => {
      ...
      const result: any = {
        ...
        is_used:
          item.is_used !== undefined
            ? item.is_used
            : attributes.is_used !== undefined
              ? attributes.is_used
              : false,
        used_at:
          item.used_at !== undefined
            ? item.used_at
            : attributes.used_at || null,
      };
  ...
  static async exportIngredients(
    sessionKey: string,
  ...
  static async importIngredients(
    sessionKey: string,
    ingredients: any[],
  ): Promise<{ created: number; ingredients: any[] }> {
  ...
  static async reimportIngredients(
    sessionKey: string,
    ingredients: any[],
  ): Promise<{
    created: number;
  ...
  static async getEventConfigs(sessionKey?: string): Promise<EventConfig[]> {
    ...
    return dataArray.map((item: any) => {
      ...
      return {
        ...
        frequency_minutes:
          item.frequency_minutes !== undefined
            ? item.frequency_minutes
            : attributes.frequency_minutes !== undefined
              ? attributes.frequency_minutes
              : null,
      };
    });
  }

  static async updateEventConfig(config: {
    event_type: string;
    enabled: boolean;
    ...
    return {
      id: (data as any).id
        ? typeof (data as any).id === "string"
          ? parseInt((data as any).id, 10)
          : Number((data as any).id)
        : attributes.id,
      event_type: attributes.event_type,
      session_id:
        attributes.session_id !== undefined ? attributes.session_id : null,
