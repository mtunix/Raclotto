...
import { Form, Input, Button, Card, Alert, Space, Upload, Avatar, Row, Col, Radio, Slider, Checkbox, message, Typography, Spin, Empty, Tag, Progress, Statistic, Rate, Modal } from 'antd';
...
import { ProfilePictureManager } from '../../shared/components/common/ProfilePictureManager';
...
export function ProfileView() {
    ...
    const { data: userProfile, isLoading: loadingProfile, error: profileError, mutate: mutateUserProfile } = useUserProfile(targetUserIdNumber);
    const { data: userStats, isLoading: loadingStats } = useUserStats(targetUserIdNumber);
    
    // Get user's pans (all sessions)
    const [offset, setOffset] = useState(0);
    const limit = 12;
    const { data: currentPageData, isLoading: loadingPans } = usePansByUser(targetUserIdNumber, limit, offset);
    ...
    const handleProfilePictureChange = async (newPicture: string | null) => {
        ...
        try {
            ...
            await Api.updateCurrentUser({ profile_picture: newPicture });
            
            // Update local user state
            const updatedUser = { ...currentUser, profile_picture: newPicture };
            setUser(updatedUser);
            
            // Re-fetch profile data to update the display immediately
            await mutateUserProfile();
            
            message.success(t("profile.pictureUpdated") || "Profile picture updated successfully");
        } catch (error: any) {
            console.error('Failed to update profile picture:', error);
            message.error(error.message || t("profile.pictureUpdateFailed") || "Failed to update profile picture");
        } finally {
