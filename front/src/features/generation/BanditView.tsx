...
import {Button, Row, Col, Form, InputNumber, Input, Checkbox, Card, Typography, Space, Alert, message, Modal, Rate} from "antd";
...
export function BanditView(props: BanditViewProps) {
    ...
    const { getAvailableFillCount, getAvailableSauceCount } = useAvailableIngredientCounts();
    ...
    const availableFills = ingredients.filter(i => 
        i.type === IngredientType.FILL && 
        i.available && 
        (i.applicable !== false)
    );
    
    const availableSauces = ingredients.filter(i => 
        i.type === IngredientType.SAUCE && 
        i.available && 
        (i.applicable !== false)
    );
    
    const totalFillCount = availableFills.length;
    const totalSauceCount = availableSauces.length;
    ...
    const hasRestrictions = availableFillCount < totalFillCount || availableSauceCount < totalSauceCount;

    // Combine all available ingredients
    const allAvailableItems = [...availableIngredients];
                ...
                <Col span={24}>
                    ...
                    >
                        ...
                        >
                            ...
                            <Button
                                onClick={() => {
                                    const newVal = Math.max(1, numColumns - 1);
                                    setNumColumns(newVal);
                                }}
                                disabled={numColumns <= 1 || isSpinning}
                                className={styles.incrementButton}
                                onMouseEnter={(e) => {
                                    if (numColumns > 1 && !isSpinning) {
                            ...
                            <Button
                                onClick={() => {
                                    const newVal = Math.min(maxColumns, numColumns + 1);
                                    setNumColumns(newVal);
                                }}
                                disabled={numColumns >= maxColumns || isSpinning}
                                className={styles.incrementButton}
                                onMouseEnter={(e) => {
                                    if (numColumns < maxColumns && !isSpinning) {
                                        e.currentTarget.style.background = '#f5f5f7';
                                        e.currentTarget.style.borderColor = '#0071e3';
                                    }
                                }}
