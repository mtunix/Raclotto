...
import "../../styles/XpNotification.css";
...
export function XpNotification() {
    ...
    const FADE_DURATION = 500; // 0.5 seconds fade, reused for auto and manual transitions

    const clearPageTimeouts = () => {
    ...
    const handleManualAdvance = () => {
        ...
        // Cancel any scheduled automatic page transitions
        clearPageTimeouts();

        if (currentPage === 'xp') {
            if (achievements.length > 0) {
                // Immediately transition from XP page to first achievement
                setPageOpacity(0);
                ...
                // No achievements to show, simply hide the notification
                setPageOpacity(0);
                fadeTimeoutRef.current = setTimeout(() => {
                    hideXpNotification();
                }, FADE_DURATION);
            ...
            if (nextIndex < achievements.length) {
                ...
                // Last achievement – hide the notification
                setPageOpacity(0);
                fadeTimeoutRef.current = setTimeout(() => {
                    hideXpNotification();
                }, FADE_DURATION);
    ...
    const notificationContent = (
        <div className={`xp-notification-container ${xpNotification.visible ? 'visible' : ''}`}>
            <Card
                className={`xp-notification-card ${
